-- Migration 045: filter get_nearby_venues by venues.discovery_approved
--
-- Change: the only difference from migration 040 is one new line in the
-- WHERE clause:
--   AND v.discovery_approved = true
--
-- This stops venues flagged as 'exclude' by the venue-review system (spam,
-- adult/gambling, no category) from appearing on the map / nearby list.
--
-- Everything else is identical to migration 040:
--   - same signature (no DROP needed — return type is unchanged)
--   - same RETURNS TABLE
--   - cover_photo_url still COALESCE(approved user photo, venues.image_url)
--   - is_published / moderation_status filters unchanged
--   - PostGIS distance, category, age, price, open-now, facility, premium
--     filters all unchanged.
--
-- Because discovery_approved defaults to true, this is a no-op for every
-- venue except those explicitly excluded — no venue silently vanishes.
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      -- Prefer an approved user-uploaded photo; fall back to the Wikimedia
      -- or category fallback image stored directly on the venues row.
      COALESCE(
        (
          SELECT vp.url
    ...
    WHERE
      ...
      AND v.discovery_approved = true   -- NEW (migration 045): hide review-excluded venues
