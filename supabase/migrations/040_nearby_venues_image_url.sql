-- Migration 040: Add image_url to get_nearby_venues cover_photo_url output
--
-- Change: cover_photo_url returned by the RPC is now
--   COALESCE(approved user photo, venues.image_url)
--
-- This means VenueCard in list / map view automatically displays Wikimedia
-- images for venues with no user-uploaded photo, with no app code changes.
-- User-uploaded approved photos always win — they are the first COALESCE arg.
--
-- Signature is unchanged (same RETURNS TABLE, same parameter list) so
-- existing callers (useNearbyVenues) need no modification.
...
BEGIN
  ...
  FROM (
    SELECT
      ...
      -- This COALESCE is the only change from migration 028.
      COALESCE(
        (
          SELECT vp.url
