-- ============================================================
-- Migration 019: Return latitude/longitude as float8 from get_nearby_venues
-- ============================================================
--
-- WHY
-- ---
-- The previous RPC declared latitude and longitude as `numeric`. PostgreSQL's
-- `numeric` type is arbitrary-precision — it has no native JavaScript
-- equivalent, so the Supabase JS client serialises it as a STRING to preserve
-- exact precision (e.g. "53.4808" rather than 53.4808).
--
-- Downstream, `react-native-maps` requires numeric lat/lng on its `<Marker
-- coordinate={...} />` prop. String coordinates cause markers to render once
-- and then silently disappear on the next map re-render / region change —
-- producing the "pins appear briefly then vanish" bug on the map tab.
--
-- FIX
-- ---
-- Cast to float8 (double precision) in the RETURN TABLE definition. float8
-- serialises as a native JS number across every Supabase client. We keep the
-- DB storage as decimal(9,6) inside the `venues` table — this change only
-- affects the wire format returned by this specific RPC.
--
-- The JS hook (hooks/useVenues.ts) also coerces defensively with Number(),
-- so this SQL change is belt-and-braces and does not require a coordinated
-- app release.
--
-- APPROACH
-- --------
-- Drop the 11-parameter overload from migration 013, then recreate with the
-- same body but float8 lat/lng columns. We cannot use `CREATE OR REPLACE`
-- because changing the RETURNS TABLE signature requires a drop first.
-- ============================================================
...
RETURNS TABLE (
  ...
  city            text,
  postcode        text,
  latitude        float8,    -- was numeric; float8 serialises as a JS number
  longitude       float8,    -- was numeric; float8 serialises as a JS number
  price_range     text,
  min_age         int,
  max_age         int,
  is_premium      boolean,
...
BEGIN
  ...
  SELECT
    ...
    sub.latitude::float8,    -- explicit cast so no implicit numeric sneaks through
