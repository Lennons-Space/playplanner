...
// ─── ExploreScreen ──────────────────────────────────────────────────────────
// Four consent states:
//   1. consentChecked=false — still reading SecureStore, render nothing
//   2. consentChecked=true, consented=false, declined=false — show consent prompt
//   3. consentChecked=true, consented=false, declined=true  — fallback map
//   4. consentChecked=true, consented=true  — live map with GPS
export default function ExploreScreen() {
  ...
  useEffect(() => {
    async function checkStoredConsent() {
      try {
        ...
        if (stored === CONSENT_VALUE) setConsented(true);
      } catch {
        // SecureStore failure — default to not-consented, prompt on next open.
      } finally {
  ...
  async function handleConsentAccept() {
    try {
      ...
      // Non-blocking GDPR Art.7 audit record — failure must never block the user.
      recordLocationConsentGranted().catch((err: unknown) => {
        console.warn('PlayPlanner: location consent logging failed:', err);
      });
    } catch {
      // If SecureStore write fails, consent works for this session only.
    }
    setConsented(true);
  }

  function handleConsentDecline() {
    // Not persisted — user is asked again on next app open per ICO guidance.
    setDeclined(true);
  }

  // State 1: still reading SecureStore — render nothing to avoid consent prompt flash.
  if (!consentChecked) return <View style={{ flex: 1, backgroundColor: Colors.slate }} />;

  // State 2: consent not yet given — show plain-English prompt first.
  if (!consented && !declined) {
    return (
      <LocationConsentPrompt onAccept={handleConsentAccept} onDecline={handleConsentDecline} />
    );
  }
